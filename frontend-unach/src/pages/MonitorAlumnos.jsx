...
export default function MonitorAlumnos() {
  ...
  useEffect(() => {
    const fetchAlumnos = async () => {
      try {
        ...
        const res = await axios.get('http://100.31.39.219:8002/api/lista-alumnos', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } });
        setAlumnos(res.data || []);
      } catch (err) {
  ...
  const descargarPDFPorCurso = (cursoId) => {
    ...
    const url = `http://100.31.39.219:8002/api/generar-pdf?cursoId=${cursoId}`;
  ...
  const enviarCorreoPDF = async (e) => {
    ...
    try {
      ...
      await axios.post('http://100.31.39.219:8002/api/enviar-reporte-correo', payload, { headers: { 'Authorization': `Bearer ${token}` } });
